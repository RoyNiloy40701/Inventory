...
const fetchUnits = async () => {
	try {
		const response = await fetch(api.allCategory, {
			method: "GET",
			headers: {
...
export const CategoryList: React.FC = () => {
	...
	const filteredCategory = category.filter((category) => {
		...
		const matchesCategory =
			...
			category.status?.toLowerCase() === selectedCategory.toLowerCase();
											...
											<div>
												<div className="text-sm font-medium text-gray-900 dark:text-white">
													<span style={{ whiteSpace: "pre-line" }}>
														{formatDateTime(category?.regdate)}
													</span>
												</div>
											</div>
										</div>
									...
									<td className="px-6 py-4 whitespace-nowrap">
										<div className="flex items-center">
											<div>
												<div className="text-sm font-medium text-gray-900 dark:text-white">
													<span style={{ whiteSpace: "pre-line" }}>
														{formatDateTime(category?.update)}
													</span>
												</div>
											</div>
										</div>
