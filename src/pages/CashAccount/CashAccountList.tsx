...
const fetchCashAccount = async () => {
	try {
		const response = await fetch(api.allCashAccount, {
			method: "GET",
			headers: {
...
export const CashAccountList: React.FC = () => {
	...
	const deleteCashAccount = async (id: number) => {
		try {
			const response = await fetch(`${api.cashAccount}/${id}`, {
				method: "DELETE",
				headers: {
					"Content-Type": "application/json",
					Authorization: `Bearer ${localStorage.getItem("auth_token")}`,
			...
			setCashAccount((prevCashAccount) =>
				prevCashAccount.filter((cashAc) => cashAc._id !== id)
			);
		} catch (error) {
	...
	useEffect(() => {
		const loadCashAccount = async () => {
			const data = await fetchCashAccount();
			setCashAccount(data);
		};
		loadCashAccount();
	}, []);

	const filteredCashAccount = cashAccount.filter((cashAc) => {
		const term = searchTerm.toLowerCase();
		const matchesSearch =
			cashAc.accountName?.toLowerCase().includes(term) ||
			cashAc.balance?.toLowerCase().includes(term) ||
			cashAc.status?.toLowerCase().includes(term);

		const matchesCategory =
			selectedCategory === "all" ||
			cashAc.status?.toLowerCase() === selectedCategory.toLowerCase();
				...
				<div className="flex items-center space-x-4">
					...
					<div>
						...
						<p className="text-gray-600 dark:text-gray-400">
							Manage your cash account List
						</p>
					</div>
				</div>
				<button
					onClick={() => navigate("/cashAccount/new")}
				...
				<div className="overflow-x-auto">
					<table className="w-full">
						...
						<tbody className="divide-y divide-gray-200 dark:divide-gray-700">
							{filteredCashAccount?.map((cashAc, index) => (
								<motion.tr
									key={cashAc._id}
									initial={{ opacity: 0, y: 20 }}
									animate={{ opacity: 1, y: 0 }}
								...
								>
									...
									<td className="px-6 py-4 text-sm font-medium text-right whitespace-nowrap">
										<div className="flex items-center justify-end space-x-2">
											...
											<button
												onClick={() =>
													navigate(`/cashAccount/${cashAc?._id}/edit`)
												}
											...
											<button
												onClick={() => deleteCashAccount(cashAc?._id)}
