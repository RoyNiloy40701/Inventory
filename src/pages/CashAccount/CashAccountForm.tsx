...
const schema = yup.object({
	name: yup.string().required("Cash Account name is required"),
...
export const CashAccountForm: React.FC = () => {
	...
	const onSubmit = async (data: FormData) => {
		try {
			...
			const url = isEdit ? `${api.cashAccount}/${id}` : api.cashAccount;
	...
	return (
		<div className="space-y-6">
			...
			>
				<button
					onClick={() => navigate("/system/cashAccount")}
				...
				<div>
					<h1 className="text-3xl font-bold text-gray-900 dark:text-white">
						{isEdit ? "Edit Cash Account Type" : "Add Cash Account Type"}
					</h1>
					<p className="text-gray-600 dark:text-gray-400">
						{isEdit
							? "Update cash account information"
							: "Enter cash account details to add"}
					</p>
				</div>
			</motion.div>
