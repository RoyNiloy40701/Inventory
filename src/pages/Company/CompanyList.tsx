...
export const CompanyList: React.FC = () => {
	...
	const deleteCompany = async (id: number) => {
		try {
			...
			setCompany((prevCompany) =>
				prevCompany.filter((company) => company._id !== id)
			);
		} catch (error) {
	...
	const filteredCompany = company.filter((company) => {
		...
		const matchesSearch =
			company.com_name?.toLowerCase().includes(term) ||
			company.com_address?.toLowerCase().includes(term) ||
			company.com_email?.toLowerCase().includes(term) ||
			company.com_mobile?.toString().includes(term) ||
			company.com_vat?.toString().includes(term);

		const matchesCategory =
			selectedCategory === "all" ||
			company.com_vat?.toLowerCase() === selectedCategory.toLowerCase();
									...
									<td className="px-6 py-4 whitespace-nowrap">
										<div className="flex items-center">
											<div>
												<div className="text-sm font-medium text-gray-900 dark:text-white">
													<span style={{ whiteSpace: "pre-line" }}>
														{formatDateTime(company?.regdate)}
													</span>
												</div>
											</div>
										</div>
