...
const fetchPurchase = async () => {
    try {
        const response = await fetch(api.allSalePayment, {
            method: "GET",
            headers: {
...
export const DuePaymentReport: React.FC = () => {
    const [dateRange, setDateRange] = useState("6months");
    const navigate = useNavigate();
    const [searchTerm, setSearchTerm] = useState("");
    const { company, customers, loading } = useDropdowns();
    ...
    useEffect(() => {
        const filtered = purchase.filter((p) => {
            const saleDate = new Date(p.regdate);
            saleDate.setHours(0, 0, 0, 0); // ignore time

            const purchaseMonth = saleDate.getMonth() + 1;
            const purchaseYear = saleDate.getFullYear();
            ...
            const matchesDateRange =
                ...
                !startDateOnly ||
                !endDateOnly ||
                (saleDate >= startDateOnly && saleDate <= endDateOnly);
            ...
            const matchesCustomer =
                !customerName ||
                p.customerName === customers?.find((s) => s._id === customerName)?.customerName;
            ...
            const matchesSearch =
                ...
                p.cMobile?.toLowerCase().includes(search) ||
                formatDateTime(p.regdate)?.toLowerCase().includes(search) ||
