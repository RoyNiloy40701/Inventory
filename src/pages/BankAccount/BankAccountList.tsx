...
export const BankAccountList: React.FC = () => {
	...
	const deleteBankAccount = async (id: number) => {
		try {
			const response = await fetch(`${api.bankAccount}/${id}`, {
				method: "DELETE",
				headers: {
					"Content-Type": "application/json",
					Authorization: `Bearer ${localStorage.getItem("auth_token")}`,
			...
			setBankAccount((prevBankAccount) =>
				prevBankAccount.filter((bankAc) => bankAc._id !== id)
			);
		} catch (error) {
	...
	const filteredMobileAccount = bankAccount.filter((bankAc) => {
		const term = searchTerm.toLowerCase();
		const matchesSearch =
			bankAc.accountName?.toLowerCase().includes(term) ||
			bankAc.bankName?.toLowerCase().includes(term) ||
			bankAc.branchName?.toLowerCase().includes(term) ||
			bankAc.accountNo?.toString().includes(term) ||
			bankAc.status?.toLowerCase().includes(term);

		const matchesCategory =
			selectedCategory === "all" ||
			bankAc.status?.toLowerCase() === selectedCategory.toLowerCase();
				...
				<div className="flex items-center space-x-4">
					...
					<div>
						...
						<p className="text-gray-600 dark:text-gray-400">
							Manage your bank account List
						</p>
					</div>
				</div>
				<button
					onClick={() => navigate("/bankAccount/new")}
				...
				<div className="overflow-x-auto">
					<table className="w-full">
						...
						<tbody className="divide-y divide-gray-200 dark:divide-gray-700">
							{filteredMobileAccount?.map((bankAc, index) => (
								<motion.tr
									key={bankAc._id}
									initial={{ opacity: 0, y: 20 }}
									animate={{ opacity: 1, y: 0 }}
								...
								>
									...
									<td className="px-6 py-4 text-sm font-medium text-right whitespace-nowrap">
										<div className="flex items-center justify-end space-x-2">
											...
											<button
												onClick={() =>
													navigate(`/bankAccount/${bankAc?._id}/edit`)
												}
											...
											<button
												onClick={() => deleteBankAccount(bankAc?._id)}
