...
const schema = yup.object({
	name: yup.string().required("Bank Account name is required"),
...
export const BankAccountForm: React.FC = () => {
	...
	const onSubmit = async (data: FormData) => {
		try {
			...
			const url = isEdit ? `${api.bankAccount}/${id}` : api.bankAccount;
	...
	return (
		<div className="space-y-6">
			...
			>
				<button
					onClick={() => navigate("/system/bankAccount")}
				...
				<div>
					<h1 className="text-3xl font-bold text-gray-900 dark:text-white">
						{isEdit ? "Edit Bank Account Type" : "Add Bank Account Type"}
					</h1>
					<p className="text-gray-600 dark:text-gray-400">
						{isEdit
							? "Update bank account information"
							: "Enter bank account details to add"}
					</p>
				</div>
			</motion.div>
