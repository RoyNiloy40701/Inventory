...
const fetchExpenses = async () => {
	try {
		const response = await fetch(api.allExpense, {
			method: "GET",
			headers: {
...
export const ExpenseList: React.FC = () => {
	...
	const filteredExpenses = expenses.filter((expense) => {
		...
		const matchesSearch =
			expense.costName?.toLowerCase().includes(term) ||
			expense.status?.toLowerCase().includes(term);

		const matchesCategory =
			selectedCategory === "all" ||
			expense.status?.toLowerCase() === selectedCategory.toLowerCase();
								...
								>
											...
											<div>
												<div className="text-sm font-medium text-gray-900 dark:text-white">
													<span style={{ whiteSpace: "pre-line" }}>
														{formatDateTime(expense?.regdate)}
													</span>
												</div>
											</div>
										</div>
									...
									<td className="px-6 py-4 text-sm font-medium text-right whitespace-nowrap">
										<div className="flex items-center justify-end space-x-2">
											...
											<button
												onClick={() =>
													navigate(`/expense/${expense?._id}/edit`)
												}
